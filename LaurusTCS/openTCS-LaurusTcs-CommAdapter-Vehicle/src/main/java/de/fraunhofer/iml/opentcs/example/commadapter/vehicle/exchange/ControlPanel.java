...
public class ControlPanel
   ...
   */
  private LarusProcessModelTO processModel;
   ...
   */
  @Inject
  public ControlPanel(@Assisted LarusProcessModelTO processModel,
  ...
  private void initGuiContent() {
    ...
    for (LarusProcessModel.Attribute attribute : LarusProcessModel.Attribute.values()) {
      processModelChange(attribute.name(), processModel);
    }
  }
  ...
  public void processModelChange(String attributeChanged, VehicleProcessModelTO newProcessModel) {
    if (!(newProcessModel instanceof LarusProcessModelTO)) {
      return;
    }
    processModel = (LarusProcessModelTO) newProcessModel;
    ...
    else if (Objects.equals(attributeChanged, LarusProcessModel.Attribute.VEHICLE_HOST.name())) {
      updateVehicleHost(processModel.getVehicleHost());
    }
    else if (Objects.equals(attributeChanged, LarusProcessModel.Attribute.VEHICLE_PORT.name())) {
      updateVehiclePort(processModel.getVehiclePort());
    }
    else if (Objects.equals(attributeChanged,
                            LarusProcessModel.Attribute.VEHICLE_IDLE_TIMEOUT.name())) {
      updateVehicleTimeout(processModel.getVehicleIdleTimeout());
    }
    else if (Objects.equals(attributeChanged, LarusProcessModel.Attribute.VEHICLE_IDLE.name())) {
      updateVehicleIdle(processModel.isVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            LarusProcessModel.Attribute.DISCONNECTING_ON_IDLE.name())) {
      updateDisconnectingOnIdle(processModel.isDisconnectingOnVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            LarusProcessModel.Attribute.RECONNECTING_ON_CONNECTION_LOSS.name())) {
      updateReconnectingOnConnectionLoss(processModel.isReconnectingOnConnectionLoss());
    }
    else if (Objects.equals(attributeChanged, LarusProcessModel.Attribute.LAST_ORDER.name())) {
      updateLastOrder(processModel.getLastOrderSent(), processModel.isCommAdapterConnected());
    }
    else if (Objects.equals(attributeChanged,
                            LarusProcessModel.Attribute.LOGGING_ENABLED.name())) {
